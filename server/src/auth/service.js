...
function parseRefreshTokenState(state) {
  ...
  try {
    ...
    // Backward compatibility for old tokens where redis value was just user id.
  }
...
export async function findUserByPhone(phoneNo, role) {
  ...
  for (const candidateRole of rolesToTry) {
    ...
    const res = await pool.query(sql, [phoneNo]);
...
export async function refreshAccessToken(refreshToken) {
  ...
  await redisClient.set(refreshTokenKey(newRefresh), serializeRefreshTokenState(user), {
    EX: REFRESH_DAYS * 24 * 60 * 60,
  });
